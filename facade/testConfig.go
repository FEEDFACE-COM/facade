...
func (config *TestConfig) Width() (uint, bool) {
	ret, ok := (*config)[testWidth].(float64)
	return uint(ret), ok
}
func (config *TestConfig) Height() (uint, bool) {
	ret, ok := (*config)[testHeight].(float64)
	return uint(ret), ok
}
func (config *TestConfig) Buffer() (BufferName, bool) {
	ret, ok := (*config)[testBuffer].(string)
	return BufferName(ret), ok
}
func (config *TestConfig) BufLen() (uint, bool) {
	ret, ok := (*config)[testBufLen].(float64)
	return uint(ret), ok
}
func (config *TestConfig) Speed() (float64, bool) {
	ret, ok := (*config)[testSpeed].(float64)
	return ret, ok
}
...
func (config *TestConfig) ApplyConfig(cfg *TestConfig) {
	if tmp, ok := cfg.Width(); ok {
		config.SetWidth(tmp)
	}
	if tmp, ok := cfg.Height(); ok {
		config.SetHeight(tmp)
	}
	if tmp, ok := cfg.Speed(); ok {
		config.SetSpeed(tmp)
	}
}
...
func (state *TestState) ApplyConfig(config *TestConfig) bool {
	changed := false
	if tmp, ok := config.Width(); ok {
		if state.Width != tmp {
			changed = true
		}
		state.Width = tmp
	}
	if tmp, ok := config.Height(); ok {
		if state.Height != tmp {
			changed = true
		}
		state.Height = tmp
	}
	if tmp, ok := config.Buffer(); ok {
		if state.Buffer != tmp {
			changed = true
		}
		state.Buffer = tmp
	}
	if tmp, ok := config.BufLen(); ok {
		if state.BufLen != tmp {
			changed = true
		}
		state.BufLen = tmp
	}
	if tmp, ok := config.Speed(); ok {
		if state.Speed != tmp {
			changed = true
		}
		state.Speed = tmp
