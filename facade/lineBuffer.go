...
func (buffer *LineBuffer) scrollOnce(freshLine bool) {
	...
	if DEBUG_LINEBUFFER {
		log.Debug("%s scroll %.2f%s", buffer.Desc(), speed, tmp)
	}
...
func (buffer *LineBuffer) pushLine(row Line) {
	...
	buffer.buf[r] = &row
	if DEBUG_LINEBUFFER {
		log.Debug("%s push #%d", buffer.Desc(), r)
	}
	...
	if buffer.buf[buffer.rows] == nil { //first offscreen slot available
		...
		if idx < total {
			...
			if buffer.Drop {
				...
				if DEBUG_LINEBUFFER {
					log.Debug("%s overflow, line jumped", buffer.Desc())
				}
				buffer.pushLine(row)
...
func (buffer *LineBuffer) Resize(newRows, newOffs uint) {
	//lock lock lock
	if DEBUG_LINEBUFFER {
		log.Debug("%s resize %d+%d", buffer.Desc(), newRows, newOffs)
	}
